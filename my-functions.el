...
(defun my-buffer-face-mode-text ()
  ...
  (setq writeroom-width 100)
  ;; (setq buffer-face-mode-face '(:extend t :family "Iosevka Comfy Duo"))
  (setq buffer-face-mode-face '(:extend t :family "ETBembo"))
  (buffer-face-mode)
...
(defun my/org-export-finalize ()
  ...
  (let* ((target-directory (expand-file-name "~/org-export"))
         (odt-files (directory-files denote-directory t "\\.odt$")))

    ;; Ensure the target directory exists
    (unless (file-exists-p target-directory)
      (make-directory target-directory t))

    ;; Process each .odt file: Convert to .docx and then process with Pandoc
    (dolist (odt-file odt-files)
      (let ((docx-file (concat target-directory "/" (file-name-base odt-file) ".docx")))
        ;; Convert with LibreOffice
        (shell-command (concat "soffice --headless --convert-to docx "
                               "--outdir " target-directory " " odt-file))
        ;; Process with Pandoc
        (shell-command (concat "pandoc -i " docx-file " -o " docx-file))))
