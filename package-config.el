...
(use-package org
  ...
  (setq
   ...
   org-todo-keywords '(
                       (sequence "TODO(t)" "NEXT(n)" "HOLD" "SKIM(s)" "READ(r)" "NOTE(N)" "|" "DONE(d)")
                       (sequence "[ ](T)" "[-](S)" "[?](W)" "|" "[X](D)"))
...
(custom-set-faces
 '(org-verbatim ((t (:inherit modus-themes-mark-alt)))))
...
(use-package! captain
  ...
  (defun my/captain-predicate ()
    "Custom predicate for Captain to avoid capitalizing after certain abbreviations."
    (and (captain-predicate-default)
         (not (looking-back "\\(etc\\.\\|e\\.g\\.\\|i\\.e\\.\\) " (- (point) 6)))
       (not (and (derived-mode-p 'org-mode) (org-in-src-block-p)))))

(defun my/captain-predicate ()
  "Custom predicate for Captain: Avoid capitalizing after certain abbreviations."
  (and (captain-predicate-default)
       ;; Checks for abbreviations. Adjust the character limit as needed.
       (not (looking-back "\\(etc\\.\\|e\\.g\\.\\|i\\.e\\.\\)\\>" (- (point) 10)))))



  (setq captain-predicate 'my/captain-predicate)
...
(use-package! xeft
    ...
    (setq xeft-directory "~/notes")
    (set-face-attribute 'xeft-inline-highlight nil :inherit 'xeft-preview-highlight)
    (add-hook! xeft-mode #'+zen/toggle)
    (map! :map xeft-mode-map
                :nvi "C-k" nil
                :nvi "C-j" nil
                )
    (map! :map xeft-mode-map
                :nvi "C-k" #'xeft-previous
                :nvi "C-j" #'xeft-next
                )
  )
...
;; Add Note to keyword higlights
(setq hl-todo--regexp "\\(\\<\\(TODO\\|FIXME\\|REVIEW\\|HACK\\|DEPRECATED\\|NOTE\\|Note\\|Question\\|BUG\\|XXX\\)\\>[:]*\\)")
(setq hl-todo-keyword-faces '(("TODO" warning bold) ("FIXME" error bold) ("REVIEW" font-lock-keyword-face bold) ("HACK" font-lock-constant-face bold) ("DEPRECATED" font-lock-doc-face bold) ("NOTE" success bold) ("Note" success bold) ("Question" warning bold) ("BUG" error bold) ("XXX" font-lock-constant-face bold)))
